...
*				3.使用VS2013以上版本编译通过
*
*	Linux：		1.库文件和头文件在同一目录下时， g++ demo_xmd_stock.cpp -o demo -L. -lxmdapi
		...
		CTORATstpRspInfoField* pRspInfoField, int nRequestID)
	{
		if (pRspInfoField->ErrorID == 0)
		{
			...
			***********************************************************************/
			...
			sub_arr_list_sh[0] = (char*)"600519";
			sub_arr_list_sh[1] = (char*)"113537";

			sub_arr_list_sz[0] = (char*)"300750";
			sub_arr_list_sz[1] = (char*)"123029";
			
			int ret_sh = m_api->SubscribeMarketData(sub_arr_list_sh, sizeof(sub_arr_list_sh) / sizeof(char*), TORA_TSTP_EXD_SSE);
			if (ret_sh != 0)
			{
				printf("SubscribeMarketData::fail, ret[%d]\n", ret_sh);
			}
...
/**************************以下为主要回调函数*****************************/
	...
	{
			printf("OnRtnMarketData::security_id[%s] security_name[%s] ExchangeID[%c] upper_limit_price[%.2f] lower_limit_price[%.2f] last_price[% .2f] HighestPrice[% .2f] LowestPrice[% .2f] BidPrice1[% .2f] AskPrice1[% .2f] BidVolume1[%lld] AskVolume1[%lld] update_time[%s] HWLevel[%d]\n",
				pMarketDataField->SecurityID,
				pMarketDataField->SecurityName,//编码为GBK
				...
				pMarketDataField->HWLevel);//1-禁止级别，2-限制级别 0-正常 3-Reits风险提示
	}
	...
	virtual void OnRspInquiryMarketDataMirror(CTORATstpMarketDataField* pMarketDataField, CTORATstpRspInfoField* pRspInfoField, int nRequestID, bool bIsLast)
	{
		...
		else if(pMarketDataField && pMarketDataField->HWLevel>0)//只过滤有警示标志的记录
		{
				printf("OnRspInquiryMarketDataMirror::security_id[%s] security_name[%s] ExchangeID[%c] UpperLimitPrice[%.2f] LowerLimitPrice[%.2f] LastPrice[%.2f],HWLevel[%d]\n",
					pMarketDataField->SecurityID,
					pMarketDataField->SecurityName,
					pMarketDataField->ExchangeID,
					pMarketDataField->UpperLimitPrice,
					pMarketDataField->LowerLimitPrice,
					pMarketDataField->LastPrice,
					pMarketDataField->HWLevel);
		}
